...
def load_DIM_Products(host, db_name, user, password):
    try:
        ...
        if new_records:
            ...
            # Insert only new records that do not exist in dim_products
            existing_ids = set(product_DWH['Product_id'].unique())
            new_records_df = new_records_df[~new_records_df['Product_id'].isin(existing_ids)]

            try:
                if not new_records_df.empty:
                    load_to_postgres("dim_products", new_records_df, host, db_name, user, password)
            except Exception as e:
                logging.error(f'Error while loading new records to dim_products: {str(e)}')
